...
export default function InsightsPageClient() {
  ...
  const others = articles.filter((article) => article.slug !== featured.slug).slice(0, 6);
        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <Link href={`/insights/${featured.slug}`} className="group grid lg:grid-cols-[1.2fr_0.8fr] gap-8 rounded-[2rem] border border-[#F0F0F0] bg-[#F9FAFB] p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow">
              <div className="rounded-[1.5rem] bg-gradient-to-br from-[#1A1A2E] to-[#F97316] min-h-[280px]" />
              <div className="flex flex-col justify-center">
                ...
                <h2 className="mt-4 text-3xl text-[#1A1A2E] font-bold" style={{ fontFamily: 'var(--font-heading)' }}>{featured.title}</h2>
                <p className="mt-4 text-[#6B7280] leading-8">{featured.excerpt}</p>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 xl:grid-cols-3 gap-6">
            {others.map((article) => (
              <Link key={article.slug} href={`/insights/${article.slug}`} className="group rounded-2xl border border-[#F0F0F0] bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
                <div className="mb-5 h-48 rounded-2xl bg-gradient-to-br from-[#FFF4EE] to-[#F97316]/20" />
                <div className="text-sm text-[#F97316] font-semibold">{article.category}</div>
                <h3 className="mt-3 text-2xl font-bold text-[#1A1A2E]" style={{ fontFamily: 'var(--font-heading)' }}>{article.title}</h3>
                <p className="mt-4 text-[#6B7280] leading-7">{article.excerpt}</p>
