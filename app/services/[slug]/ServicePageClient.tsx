        ...
        <section className="py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
            <div>
              ...
              <h1 className="mt-4 text-4xl md:text-5xl text-[#1A1A2E] font-bold max-w-3xl" style={{ fontFamily: 'var(--font-heading)' }}>{service.hero}</h1>
              <p className="mt-6 text-lg leading-8 text-[#6B7280] max-w-3xl">{service.intro}</p>
            ...
            <div className={`rounded-[2rem] bg-gradient-to-br ${service.accent} min-h-[320px] border border-[#F0F0F0] p-8`}>
              <div className="grid gap-4 md:grid-cols-3 md:items-end h-full">
                {service.stats.map((stat) => (
                  <div key={stat.label} className="rounded-3xl bg-white/70 p-5 backdrop-blur-sm border border-white/50">
                    <p className="text-sm text-[#6B7280]">{stat.label}</p>
                    <p className="mt-2 text-3xl font-bold text-[#1A1A2E]" style={{ fontFamily: 'var(--font-heading)' }}>{stat.value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
              {service.process.map((step, index) => (
                <div key={step.title} className="rounded-2xl border border-[#F0F0F0] bg-[#F9FAFB] p-6 shadow-sm">
