      ...
      <main>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              {services.map((service) => {
                ...
                return (
                  <div key={service.slug} className="rounded-[2rem] border border-[#F0F0F0] bg-[#F9FAFB] p-8 shadow-sm transition-shadow hover:shadow-md dark:border-sop-border dark:bg-sop-surface dark:shadow-sop-card dark:hover:shadow-sop-purple">
                    ...
                    <h2 className="text-2xl font-bold text-[#1A1A2E] dark:text-sop-foreground" style={{ fontFamily: 'var(--font-heading)' }}>{isArabic ? service.titleAR : service.titleEN}</h2>
                    <p className="mt-4 text-[#6B7280] leading-8 dark:text-sop-muted">{isArabic ? service.descriptionAR : service.descriptionEN}</p>
                    <ul className="mt-6 grid gap-3 text-sm text-[#1A1A2E] sm:grid-cols-2 dark:text-sop-foreground">
                      {(isArabic ? service.featuresAR : service.featuresEN).slice(0, 4).map((item) => (
                        <li key={item} className="rounded-xl border border-[#F0F0F0] bg-white px-4 py-3 dark:border-sop-border dark:bg-sop-bg dark:text-sop-muted">{item}</li>
                      ))}
                    </ul>
        ...
        <section className="bg-[#F9FAFB] py-20 dark:bg-sop-surface">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
              {steps.map((step) => (
                <div key={step.number} className="rounded-2xl border border-[#F0F0F0] bg-white p-6 shadow-sm dark:border-sop-border dark:bg-sop-bg dark:shadow-sop-card">
                  <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-full bg-[#F97316] text-white font-bold dark:bg-primary dark:shadow-sop-orange">{step.number}</div>
                  <h3 className="text-xl font-bold text-[#1A1A2E]" style={{ fontFamily: 'var(--font-heading)' }}>{step.title}</h3>
                  <p className="mt-3 text-[#6B7280] leading-7 dark:text-sop-muted">{step.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="bg-[#1A1A2E] py-20 dark:bg-sop-bg">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-[1fr_0.8fr] gap-10 items-center">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8 dark:border-sop-border dark:bg-sop-surface/60">
