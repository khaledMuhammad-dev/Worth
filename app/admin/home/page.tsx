      ...
      <div className="p-6">
        <Tabs defaultValue="hero">
          ...
          <TabsContent value="services" className="space-y-4 rounded-xl border border-gray-100 bg-white p-6">
            <LocaleField labelEN="Heading" labelAR="Heading" valueEN={data.services.headingEN} valueAR={data.services.headingAR} onChangeEN={(value) => setData({ ...data, services: { ...data.services, headingEN: value } })} onChangeAR={(value) => setData({ ...data, services: { ...data.services, headingAR: value } })} />
            <LocaleField labelEN="Accent" labelAR="Accent" valueEN={data.services.accentWordEN} valueAR={data.services.accentWordAR} onChangeEN={(value) => setData({ ...data, services: { ...data.services, accentWordEN: value } })} onChangeAR={(value) => setData({ ...data, services: { ...data.services, accentWordAR: value } })} />
            {data.services.items.map((item, index) => (
              <div key={item.id} className="space-y-3 rounded-xl border border-gray-100 p-4">
                <LocaleField multiline labelEN="Title" labelAR="Title" valueEN={item.titleEN} valueAR={item.titleAR} onChangeEN={(value) => setData({ ...data, services: { ...data.services, items: data.services.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, titleEN: value } : entry) } })} onChangeAR={(value) => setData({ ...data, services: { ...data.services, items: data.services.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, titleAR: value } : entry) } })} />
                <LocaleField multiline labelEN="Description" labelAR="Description" valueEN={item.descriptionEN} valueAR={item.descriptionAR} onChangeEN={(value) => setData({ ...data, services: { ...data.services, items: data.services.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, descriptionEN: value } : entry) } })} onChangeAR={(value) => setData({ ...data, services: { ...data.services, items: data.services.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, descriptionAR: value } : entry) } })} />
          ...
          <TabsContent value="process" className="space-y-4 rounded-xl border border-gray-100 bg-white p-6">
            <LocaleField labelEN="Heading" labelAR="Heading" valueEN={data.process.headingEN} valueAR={data.process.headingAR} onChangeEN={(value) => setData({ ...data, process: { ...data.process, headingEN: value } })} onChangeAR={(value) => setData({ ...data, process: { ...data.process, headingAR: value } })} />
            <LocaleField labelEN="Accent" labelAR="Accent" valueEN={data.process.accentWordEN} valueAR={data.process.accentWordAR} onChangeEN={(value) => setData({ ...data, process: { ...data.process, accentWordEN: value } })} onChangeAR={(value) => setData({ ...data, process: { ...data.process, accentWordAR: value } })} />
            {data.process.steps.map((step, index) => (
              <div key={step.number} className="space-y-3 rounded-xl border border-gray-100 p-4">
                ...
                <LocaleField labelEN="Title" labelAR="Title" valueEN={step.titleEN} valueAR={step.titleAR} onChangeEN={(value) => setData({ ...data, process: { ...data.process, steps: data.process.steps.map((item, itemIndex) => itemIndex === index ? { ...item, titleEN: value } : item) } })} onChangeAR={(value) => setData({ ...data, process: { ...data.process, steps: data.process.steps.map((item, itemIndex) => itemIndex === index ? { ...item, titleAR: value } : item) } })} />
                <LocaleField multiline labelEN="Description" labelAR="Description" valueEN={step.descriptionEN} valueAR={step.descriptionAR} onChangeEN={(value) => setData({ ...data, process: { ...data.process, steps: data.process.steps.map((item, itemIndex) => itemIndex === index ? { ...item, descriptionEN: value } : item) } })} onChangeAR={(value) => setData({ ...data, process: { ...data.process, steps: data.process.steps.map((item, itemIndex) => itemIndex === index ? { ...item, descriptionAR: value } : item) } })} />
              </div>
            ))}
          </TabsContent>

          <TabsContent value="testimonials" className="space-y-4 rounded-xl border border-gray-100 bg-white p-6">
            <LocaleField labelEN="Heading" labelAR="Heading" valueEN={data.testimonials.headingEN} valueAR={data.testimonials.headingAR} onChangeEN={(value) => setData({ ...data, testimonials: { ...data.testimonials, headingEN: value } })} onChangeAR={(value) => setData({ ...data, testimonials: { ...data.testimonials, headingAR: value } })} />
            <LocaleField labelEN="Accent" labelAR="Accent" valueEN={data.testimonials.accentWordEN} valueAR={data.testimonials.accentWordAR} onChangeEN={(value) => setData({ ...data, testimonials: { ...data.testimonials, accentWordEN: value } })} onChangeAR={(value) => setData({ ...data, testimonials: { ...data.testimonials, accentWordAR: value } })} />
            {data.testimonials.items.map((item, index) => (
              <div key={item.id} className="space-y-3 rounded-xl border border-gray-100 p-4">
                <LocaleField labelEN="Name" labelAR="Name" valueEN={item.nameEN} valueAR={item.nameAR} onChangeEN={(value) => setData({ ...data, testimonials: { ...data.testimonials, items: data.testimonials.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, nameEN: value } : entry) } })} onChangeAR={(value) => setData({ ...data, testimonials: { ...data.testimonials, items: data.testimonials.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, nameAR: value } : entry) } })} />
                <LocaleField labelEN="Role" labelAR="Role" valueEN={item.roleEN} valueAR={item.roleAR} onChangeEN={(value) => setData({ ...data, testimonials: { ...data.testimonials, items: data.testimonials.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, roleEN: value } : entry) } })} onChangeAR={(value) => setData({ ...data, testimonials: { ...data.testimonials, items: data.testimonials.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, roleAR: value } : entry) } })} />
                <LocaleField multiline labelEN="Quote" labelAR="Quote" valueEN={item.quoteEN} valueAR={item.quoteAR} onChangeEN={(value) => setData({ ...data, testimonials: { ...data.testimonials, items: data.testimonials.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, quoteEN: value } : entry) } })} onChangeAR={(value) => setData({ ...data, testimonials: { ...data.testimonials, items: data.testimonials.items.map((entry, itemIndex) => itemIndex === index ? { ...entry, quoteAR: value } : entry) } })} />
              </div>
            ))}
          </TabsContent>

          <TabsContent value="cta" className="space-y-4 rounded-xl border border-gray-100 bg-white p-6">
            <LocaleField multiline labelEN="Heading" labelAR="Heading" valueEN={data.cta.headingEN} valueAR={data.cta.headingAR} onChangeEN={(value) => setData({ ...data, cta: { ...data.cta, headingEN: value } })} onChangeAR={(value) => setData({ ...data, cta: { ...data.cta, headingAR: value } })} />
            <LocaleField labelEN="Button" labelAR="Button" valueEN={data.cta.buttonEN} valueAR={data.cta.buttonAR} onChangeEN={(value) => setData({ ...data, cta: { ...data.cta, buttonEN: value } })} onChangeAR={(value) => setData({ ...data, cta: { ...data.cta, buttonAR: value } })} />
